...
alter table public.price_variants add column if not exists updated_at timestamptz not null default now();
...
create policy "anon_read_shops" on public.shops for select using (true);

drop policy if exists "anon_write_shops" on public.shops;
create policy "anon_write_shops" on public.shops for all using (true) with check (true);
...
-- OPTIONAL storage section (run separately if your role can manage storage.* objects)
