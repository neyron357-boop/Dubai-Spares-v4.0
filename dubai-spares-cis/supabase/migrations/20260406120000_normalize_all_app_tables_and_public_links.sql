-- Canonical idempotent normalization for all app tables used by web + public links.
-- Safe for reruns. Restores missing tables/columns and refreshes PostgREST schema cache.
...
grant all on table public.app_state, public.orders, public.parts, public.price_variants, public.shops, public.push_subscriptions, public.public_quote_snapshots, public.client_leads, public.backups, public.backups_meta to anon, authenticated;
...
create policy "quote_update_anon" on public.public_quote_snapshots for update to anon using (true) with check (true);
...
-- Force schema cache refresh right after migration.
