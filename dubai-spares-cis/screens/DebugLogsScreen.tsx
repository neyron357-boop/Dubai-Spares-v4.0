    ...
    <div className="p-4 pb-24 space-y-3 overflow-x-hidden">
      ...
      {tab === 'diagnostic' && (
        <div className="space-y-2 text-xs">
          ...
          <div className="rounded-xl border bg-white p-3 space-y-1"><div className="font-black">Sync diagnostics</div><p>last sync: {perfSnapshot.lastSyncAt ? formatTime(perfSnapshot.lastSyncAt) : '—'}</p><p>outbox length: {perfSnapshot.queueLength}</p><p>retry count: {perfSnapshot.retryCount}</p><p>active request: {perfSnapshot.activeRequest ? 'yes' : 'no'}</p><p>last error type: {perfSnapshot.lastErrorType || '—'}</p><p>next retry in: {perfSnapshot.nextRetryAt && perfSnapshot.nextRetryAt > Date.now() ? `${Math.ceil((perfSnapshot.nextRetryAt - Date.now()) / 1000)}s` : '—'}</p><p>last error: {perfSnapshot.lastError || '—'}</p><p>schema warnings: {perfSnapshot.schemaWarnings.length ? perfSnapshot.schemaWarnings.join(', ') : 'none'}</p><p>typing avg: {perfSnapshot.typingAvgMs} ms</p><p>IDB writes/sec: {perfSnapshot.idbWritesPerSecond}</p><p>IDB open attempts: {perfSnapshot.dbOpenAttempts}</p><p>IDB db version: {perfSnapshot.dbVersion || '—'}</p><p>IDB tx/sec: {perfSnapshot.txCountPerSecond}</p><p>IDB avg tx time: {perfSnapshot.avgTxTimeMs} ms</p><p>last IDB error: {perfSnapshot.lastIdbError || '—'}</p><p>network req/sec: {perfSnapshot.networkRequestsPerSecond}</p><p>last request: {perfSnapshot.lastNetworkRequest ? `${formatTime(perfSnapshot.lastNetworkRequest.at)} · ${perfSnapshot.lastNetworkRequest.operation}${perfSnapshot.lastNetworkRequest.orderId ? ` · ${perfSnapshot.lastNetworkRequest.orderId}` : ''}${perfSnapshot.lastNetworkRequest.bytes ? ` · ${perfSnapshot.lastNetworkRequest.bytes} bytes` : ''}` : '—'}</p><p>last 20 IDB events:</p><ul className="list-disc pl-4 max-h-36 overflow-y-auto">{perfSnapshot.idbEvents.map((event) => <li key={`${event.at}-${event.event}`}>{formatTime(event.at)} · {event.event}</li>)}</ul><button className="rounded-lg bg-amber-600 text-white px-2 py-1 text-[11px] font-black" type="button" onClick={async () => { if (!window.confirm('Rebuild local cache? This will export local orders + sync queue, recreate IndexedDB, and restore local data.')) return; await offlineDb.rebuildLocalCacheSafely(); window.location.reload(); }}>Rebuild local cache</button></div>
