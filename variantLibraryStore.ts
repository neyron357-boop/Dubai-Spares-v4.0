...
const normalizeStandaloneVariant = (item: VariantLibraryItem): VariantLibraryItem => ({
  ...
  vin: typeof item.vin === 'string' ? item.vin.trim() : '',
    ...
    order.parts.flatMap((part) =>
      (part.variants || []).map((variant) => {
        const carTitle = [order.brand, order.model, order.year]
          ...
          .filter(Boolean)
          .join(' ');
        const orderVin = String(order.vin || '').trim();
        const variantVehicleInfo = String(variant.vehicleInfo || '').trim();
        const variantVin = String(variant.vin || '').trim();
        return {
          ...variant,
