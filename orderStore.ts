  ...
  const parts = Array.isArray(order.parts)
    ? order.parts.map((part) => {
      ...
      const groupItems = Array.isArray((part as any).groupItems)
        ...
        : [];
      const partKind: 'single' | 'group' = (part as any).partKind === 'group' ? 'group' : 'single';
      return {
        ...part,
        quantity: normalizePartQuantity((part as any).quantity),
        comment: String(part.comment || ''),
        googleDriveVideoUrl: typeof (part as any).googleDriveVideoUrl === 'string' ? (part as any).googleDriveVideoUrl.trim() : '',
        partKind,
        groupItems,
        photos: partPhotos,
...
const fetchOrdersPage = async (orderColumns: string[], from: number, to: number) => {
  ...
  return { data: ordersResponse.data as unknown as DbOrderGraphRow[], error: null };
};
...
const fetchOrdersGraphWithSchemaFallbacks = async () => {
  ...
  while (true) {
    ...
    if (!tableColumns.includes(missingIssue.column as (typeof tableColumns)[number])) {
      return response;
    }
...
const fetchOrderDetailsGraphWithSchemaFallbacks = async (orderId: string) => {
  if (!supabase) return { data: null, error: null };
  ...
  while (true) {
    ...
    const variantColumns = getSelectableColumns('price_variants');
    const partsJoin = `parts(${partColumns.join(',')}, price_variants(${variantColumns.join(',')}))`;
    const query = `${orderColumns.join(',')}, ${partsJoin}`;
    const response = await supabase.from('orders').select(query).eq('id', orderId).maybeSingle();

    if (!response.error) {
      return { data: response.data as DbOrderGraphRow | null, error: null };
    }
    ...
    if (isNewlyMissing) {
      ...
      syncPerf.addSchemaWarning(schemaKey);
      logSyncCategory('SCHEMA_MISMATCH', 'details_column_missing', { table: missingIssue.table, column: missingIssue.column });
      await logger.warn('sync:fetch-details', `schema_missing_columns: ["${schemaKey}"]`, { orderId });
    }
...
const mapDbOrder = (row: DbOrderGraphRow): Order => ({
  ...normalizeOrder({
    ...
    googleDriveFolderUrl: typeof (row as any).google_drive_folder_url === 'string' ? (row as any).google_drive_folder_url.trim() : '',
    parts: (row.parts || []).map((part) => ({
      id: String(part.id),
      orderId: String(part.order_id),
      name: part.name,
      comment: (part as any).comment || '',
      googleDriveVideoUrl: typeof (part as any).google_drive_video_url === 'string' ? (part as any).google_drive_video_url.trim() : '',
...
const persistOrderGraph = async (order: Order) => {
  ...
  const buildOrderPayload = () => ({
    ...
    google_drive_folder_url: uploadedOrder.googleDriveFolderUrl || null,
  ...
  const partRows = (cloudOrder.parts || []).map((part) => ({
    ...
    google_drive_video_url: part.googleDriveVideoUrl || null,
...
export const fetchOrderDetails = async (orderId: string) => {
  ...
  const response = await fetchOrderDetailsGraphWithSchemaFallbacks(orderId);
...
export const updateOrderItem = async (order: Order) => {
  ...
  void (async () => {
    try {
      ...
      await queueMutation('upsert', normalized, normalized.id);
      setState({ orders: state.orders, error: null });
    }
  })();
