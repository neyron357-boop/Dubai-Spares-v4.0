...
const toLinkedPartsArray = (value: unknown): Supplier['linkedParts'] => {
  ...
  return value
    .map((item): NonNullable<Supplier['linkedParts']>[number] | null => {
...
const shouldUseEnrichedSuppliers = () => {
  if ((import.meta as any).env?.VITE_ENABLE_ENRICHED_SHOPS === 'true') return true;
  try {
    return window.localStorage.getItem('dubai-spares-enable-enriched-shops') === '1';
  } catch {
    return false;
  }
};
...
export const getSuppliersEnriched = async (): Promise<Supplier[]> => {
  if (!supabase) return [];
  if (!shouldUseEnrichedSuppliers()) return fetchSuppliersFallback();
