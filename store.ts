...
const normalizeSupplier = (supplier: Supplier): Supplier => ({
  ...
  linkedParts: Array.isArray(supplier.linkedParts)
    ? supplier.linkedParts
      ...
      .map((item): NonNullable<Supplier['linkedParts']>[number] => ({
        ...item,
  ...
  interactions: Array.isArray(supplier.interactions)
    ? supplier.interactions
      ...
      .map((item): NonNullable<Supplier['interactions']>[number] => ({
        id: typeof item.id === 'string' ? item.id : ensureUuid(),
  ...
  orders.forEach((order) => {
    ...
    order.parts.forEach((part) => {
      part.variants.forEach((variant) => {
        ...
        const key = rawName.toLowerCase();
        const now = Date.now();
        const linkedPart: NonNullable<Supplier['linkedParts']>[number] = {
          id: ensureUuid(),
          orderId: order.id,
...
export const useStore = () => {
  ...
  const { orders, isLoading, isSyncing, isHydrated, error, addOrder, updateOrder, deleteOrder, bulkDeleteOrders, updatePart, removePart, updatePriceVariant, fetchOrders } = useOrderStore();
  ...
  }), [version, orders, isLoading, isSyncing, isHydrated, error, addOrder, updateOrder, deleteOrder, bulkDeleteOrders, updatePart, removePart, updatePriceVariant, addSupplier, updateSupplier, deleteSupplier, getBackupData, restoreData, fetchOrders, fetchOrderDetails, variantLibrary, saveStandaloneVariant, removeStandaloneVariant]);
};
