...
const SHARED_IMAGE_FOLDER = 'shared/images';
...
export const uploadImageToStorage = async (
  ...
  const contentHash = await sha256Hex(compressedBuffer);
  ...
  const legacyPath = `${folder}/${hasKnownExtension ? normalizedName : `${normalizedName}.${ext}`}`;
  const path = `${SHARED_IMAGE_FOLDER}/${contentHash}.${ext}`;
  ...
  for (const bucket of BUCKET_CANDIDATES) {
    try {
      ...
      if (existingResponse.ok) {
        ...
        await logger.info('storage:upload-reused', `${folder}/${fileName} reused existing image`, {
          bucket,
          path,
          contentHash,
          sizeBytes: compressed.size
      ...
      // If the existence probe is unavailable, fall through to the normal upload path.
    }
