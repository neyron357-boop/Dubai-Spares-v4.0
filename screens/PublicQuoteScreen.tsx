...
import { normalizePublicQuoteSnapshotPayload, type QuoteProofAttachment } from '../utils/publicQuoteSnapshot';
...
const isNewWindowHref = (href: string) => /^(https?:|data:|blob:)/i.test(href);
...
const resolveProofAttachmentHref = (attachment: QuoteProofAttachment) => {
  if (attachment.kind === 'location') {
    if (attachment.value) return attachment.value;
    if (Number.isFinite(attachment.latitude) && Number.isFinite(attachment.longitude)) {
      return `https://www.google.com/maps?q=${attachment.latitude},${attachment.longitude}`;
    }
    return '';
  }
  if (attachment.kind === 'contact') {
    return attachment.phone ? `tel:${attachment.phone.replace(/\s/g, '')}` : attachment.value;
  }
  return attachment.fileUrl || attachment.value;
};
...
const PublicQuoteScreen: React.FC<PublicQuoteScreenProps> = ({ orderId }) => {
  ...
  useEffect(() => {
    return () => {
      if (playingAudioId) {
        const audioEl = document.getElementById(playingAudioId) as HTMLAudioElement | null;
        audioEl?.pause();
      }
    };
  }, [playingAudioId]);
  ...
  const proofAttachmentCount = proofNotes.reduce((sum, note) => sum + note.attachments.length, 0);
  ...
  const vehicleLabel = [order.brand, order.model, String(order.year || '')].filter((value) => value && value !== '—').join(' ').trim();

  const whatsappConfirmationText = lang === 'ru'
    ? `Здравствуйте! Подтверждаю смету по ${vehicleLabel || 'моему авто'} на сумму ${(payableTotalAed * fx).toFixed(2)} ${activeCurrency}.`
    : `Hello! I confirm the quote for ${vehicleLabel || 'my car'} for ${(payableTotalAed * fx).toFixed(2)} ${activeCurrency}.`;
  ...
  const renderProofAttachment = (attachment: QuoteProofAttachment, index: number) => {
    const href = resolveProofAttachmentHref(attachment);
    const isFile = attachment.kind === 'file';
    const isLocation = attachment.kind === 'location';
    const isContact = attachment.kind === 'contact';
    const Icon = isLocation ? MapPin : isContact ? User : Paperclip;
    const label = attachment.name || (lang === 'ru' ? 'Вложение' : 'Attachment');
    const coordinates = Number.isFinite(attachment.latitude) && Number.isFinite(attachment.longitude)
      ? `${attachment.latitude}, ${attachment.longitude}`
      : '';
    const subtitle = isLocation
      ? (attachment.address || coordinates || 'Shared location')
      : isContact
      ...
      <div key={`${attachment.id}-${index}`} className="flex items-center gap-2 rounded-[22px] bg-white p-2 shadow-[0_8px_24px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/80">
        ...
        {href && (
          <a
            ...
            download={isFile && href.startsWith('data:') ? attachment.name : undefined}
            className="ds-press grid h-9 w-9 shrink-0 place-items-center rounded-full bg-slate-100 text-slate-600 hover:bg-slate-200 transition"
    ...
    <div className="min-h-[100dvh] bg-[#f4f6f8] px-3 pb-[calc(104px+env(safe-area-inset-bottom))] pt-[calc(70px+0.75rem)] sm:px-6">
      <main className="mx-auto flex max-w-5xl flex-col gap-3 sm:gap-4">
        ...
        <nav className="grid grid-cols-3 gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm" aria-label="Quote tabs">
          ...
          >
            Proof Pack
            {(proofNotes.length > 0 || proofPhotoCount > 0 || proofVideoCount > 0 || proofAudioCount > 0 || proofAttachmentCount > 0) && <span className="ml-2 rounded-full bg-white/20 px-2 py-0.5 text-[11px]">{proofNotes.length}</span>}
          </button>
          <button
            type="button"
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-center sm:grid-cols-4">
              ...
              <div className="rounded-lg bg-slate-50 px-3 py-2"><p className="text-[10px] font-bold text-slate-400">{lang === 'ru' ? 'Файлы' : 'Files'}</p><p className="text-sm font-black text-slate-900">{proofAttachmentCount}</p></div>
            </div>
          </div>
            ...
            <div className="mt-5 space-y-4">
              {proofNotes.map((note) => (
                <article key={note.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  {note.text && !(note.text === 'Attachment' && note.attachments.length > 0) && <p className="whitespace-pre-line text-sm font-semibold leading-6 text-slate-700">{note.text}</p>}
                  {note.photos.length > 0 && (
                    <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                      {note.photos.map((photo, index) => (
                        photo.startsWith('data:video/') || photo.startsWith('blob:video/') || photo.includes('video') ? (
                          <video key={`${note.id}-${photo}-${index}`} src={photo} className="aspect-square overflow-hidden rounded-2xl bg-slate-200 object-cover" controls playsInline />
                        ) : (
                          <button key={`${note.id}-${photo}-${index}`} type="button" onClick={() => setGallery({ images: note.photos, index })} className="ds-press aspect-square overflow-hidden rounded-2xl bg-slate-200">
                  ...
                  {note.attachments.length > 0 && (
                    <div className="mt-3 grid gap-2">
                      {note.attachments.map((attachment, index) => renderProofAttachment(attachment, index))}
                    </div>
                  )}
                  {note.audios.length > 0 && (
                    <div className="mt-3 space-y-2">
                      {note.audios.map((audio, index) => {
                        const audioId = `proof-${note.id}-${audio.id}-${index}`;
                        const isPlaying = playingAudioId === audioId;
                        const progress = audioProgress[audioId] || 0;
                        const bars = getWaveBars((audio.fileUrl || '').slice(0, 120));
                        return (
                          <div key={audioId} className="max-w-[92%] rounded-[24px] rounded-tl-md bg-[#D8F4E5] p-2.5 shadow-[0_8px_22px_rgba(16,185,129,0.14),inset_0_0_0_1px_rgba(16,185,129,0.08)]">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="mt-0.5 flex items-center justify-between gap-2 text-[11px] font-black text-emerald-900/60">
                                  <span>{formatSeconds(audio.duration)}</span>
                                  <span className="inline-flex items-center gap-1">
                                    {note.createdAt > 0 && new Date(note.createdAt).toLocaleTimeString(lang === 'ru' ? 'ru-RU' : 'en-US', { hour: '2-digit', minute: '2-digit' })}
                                    <Check size={12} className="text-emerald-600" />
                                    <Check size={12} className="-ml-2 text-emerald-600" />
                                  </span>
                                </div>
                              </div>
                              <audio id={audioId} src={audio.fileUrl} preload="metadata" playsInline />
                            </div>
                          </div>
                        );
                      })}
