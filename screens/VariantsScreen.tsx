...
const VariantsScreen: React.FC = () => {
  ...
  const visibleVariantLibrary = useMemo(() => {
    const orderVariantKeys = new Set(
      variantLibrary
        .filter((variant) => variant.origin === 'order')
        .map(getVariantDedupeKey)
    );
  ...
  const prepareVariantPhotos = async (files: File[], context: string) => {
    return await Promise.all(files.map(async (file) => {
      try {
        return await optimizeImageForUpload(file, `${context}:${file.name}`);
      } catch {
        const reader = new FileReader();
        return await new Promise<string>((resolve) => {
          reader.onloadend = () => resolve(String(reader.result || ''));
  ...
  const updateSelectedVariantPhotos = (nextPhotos: string[]) => {
    ...
    setSelectedVariant((prev) => prev ? { ...prev, photos: cleanPhotos, photoUrl: cleanPhotos[0] || '' } : prev);
  };
  ...
  const removeSelectedVariantPhoto = (indexToRemove: number) => {
    ...
    updateSelectedVariantPhotos(miniPhotos(selectedVariant).filter((_, index) => index !== indexToRemove));
  };
  ...
  const makeSelectedVariantPhotoPrimary = (indexToPromote: number) => {
    ...
    updateSelectedVariantPhotos([targetPhoto, ...currentPhotos.filter((_, index) => index !== indexToPromote)]);
  };
    ...
    return orders
      ...
      .flatMap((order) => {
        ...
        return [{
          ...
          vin,
          orderRef: order.id,
          meta: [partsPreview, vin ? `VIN ${vin}` : '', order.clientName || 'Активный заказ'].filter(Boolean).join(' · ')
        }];
      })
      .slice(0, 24);
  }, [orders]);
  const filteredVehicleInfoOptions = useMemo(() => {
    ...
    return vehicleInfoOptions
      .filter((option) => `${option.value} ${option.vin} ${option.meta}`.toLowerCase().includes(query))
      .slice(0, 8);
  }, [vehicleInfo, vehicleInfoOptions]);

  const filteredSelectedVehicleInfoOptions = useMemo(() => {
    const query = String(selectedVariant?.vehicleInfo || '').trim().toLowerCase();
    if (!query) return vehicleInfoOptions.slice(0, 8);
    return vehicleInfoOptions
      .filter((option) => `${option.value} ${option.vin} ${option.meta}`.toLowerCase().includes(query))
      .slice(0, 8);
  ...
  const isAttachedToExistingOrder = (variant: VariantLibraryItem) => Boolean(variant.sourceOrderId && orders.some((order) => order.id === variant.sourceOrderId));
  ...
  const attachVariantToOrder = async (orderId: string) => {
    ...
    const variantClone = { ...cloneVariantForPart(variantToAttach, partId), orderId: targetOrder.id };
                ...
                >
                  <div className="relative h-[112px] shrink-0 bg-[#F2F4F7]">
                    {firstPhoto ? (
                      ...
                      >
                        <SafeImage src={firstPhoto} alt={getVariantTitle(variant)} className="h-full w-full object-contain p-2" loading="lazy" />
                      </button>
                    ) : (
                      <SafeImage src="" alt={`Фото недоступно: ${getVariantTitle(variant)}`} className="h-full w-full object-cover" loading="lazy" />
                    )}
                    <button
                      type="button"
                      onClick={(event) => {
                  ...
                  <div className="flex gap-2 overflow-x-auto no-scrollbar">
                    {photos.map((photo, index, images) => (
                      <button
                        ...
                        className="h-16 w-16 shrink-0 overflow-hidden rounded-xl border border-[#E7EAF0] bg-[#F2F4F7]"
                        aria-label={`Открыть фото ${index + 1}`}
                      >
                        <img src={photo} className="h-full w-full object-cover" alt={`Фото варианта ${index + 1}`} />
                      </button>
                    ))}
                  </div>
                )}
      ...
      {selectedVariant && (
        <div className="fixed inset-0 z-50 flex items-end bg-black/55">
          <div className="max-h-[92dvh] w-full overflow-y-auto overscroll-contain touch-pan-y rounded-t-[24px] bg-white px-4 pb-[max(6.5rem,calc(env(safe-area-inset-bottom)+4.5rem))] pt-0">
            <input ref={detailFileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={onSelectedVariantPhotosChange} />
            {selectedVariantPhotos.length > 0 ? (
              <div className="-mx-4 relative overflow-hidden rounded-t-[24px] bg-[#F2F4F7]">
                <div className="flex snap-x snap-mandatory overflow-x-auto no-scrollbar" data-horizontal-scroll="true">
                  {selectedVariantPhotos.map((photo, index, images) => (
                    <button
                      ...
                      aria-label={`Открыть фото ${index + 1}`}
                    >
                      <SafeImage src={photo} alt={`Фото варианта ${index + 1}`} className="h-full w-full object-contain p-3" />
                    </button>
                  ))}
                </div>
                ...
                {isEditMode && (
                  <button type="button" onClick={() => detailFileInputRef.current?.click()} className="absolute bottom-3 left-3 inline-flex h-9 items-center gap-1.5 rounded-full bg-white/92 px-3 text-[11px] font-black text-[#0F1728] shadow-sm active:scale-[0.98]">
                    <Camera size={14} /> Изменить фото
                  </button>
                )}
              </div>
            ) : isEditMode ? (
              <div className="-mx-4 relative rounded-t-[24px] bg-[#F2F4F7] px-4 pb-4 pt-8">
                <div className="absolute left-1/2 top-2 h-1.5 w-10 -translate-x-1/2 rounded-full bg-black/18" />
                <button type="button" onClick={() => detailFileInputRef.current?.click()} className="flex h-36 w-full flex-col items-center justify-center gap-2 rounded-[22px] border border-dashed border-[#B8C4D6] bg-white/70 text-sm font-black text-[#475467] active:scale-[0.99]">
                  <Camera size={22} />
                  Добавить фото
                </button>
              </div>
            ...
            {isEditMode ? (
              <div className="mt-4 space-y-2">
                <section className="space-y-2 rounded-2xl border border-[#E7EAF0] p-3">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-xs font-semibold text-[#667085]">Фото</p>
                    <button type="button" onClick={() => detailFileInputRef.current?.click()} className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-[#D0D5DD] px-3 text-xs font-bold text-[#475467] active:scale-[0.98]">
                  ...
                  {selectedVariantPhotos.length > 0 ? (
                    <div className="grid grid-cols-3 gap-2">
                      {selectedVariantPhotos.map((photo, index, images) => (
                        <div key={`${photo}-${index}-edit`} className="relative overflow-hidden rounded-2xl border border-[#E7EAF0] bg-[#F2F4F7]">
                          <button
                            type="button"
                            onClick={() => setGallery({ images, index })}
                            ...
                            aria-label={`Открыть фото ${index + 1}`}
                          >
                            <SafeImage src={photo} alt={`Фото варианта ${index + 1}`} className="h-full w-full object-cover" />
                          </button>
                          <button
                            type="button"
                          ...
                          {index === 0 ? (
                            <span className="absolute bottom-1 left-1 rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-black text-emerald-700">Главная</span>
                          ) : (
                            <button
                              type="button"
                              onClick={() => makeSelectedVariantPhotoPrimary(index)}
                              className="absolute bottom-1 left-1 inline-flex h-7 items-center gap-1 rounded-full bg-white/92 px-2 text-[10px] font-black text-[#475467]"
                            >
                    ...
                    <button type="button" onClick={() => detailFileInputRef.current?.click()} className="flex min-h-24 w-full flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-[#D0D5DD] bg-[#F7F8FA] text-xs font-bold text-[#667085]">
                      <Camera size={18} />
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="rounded-2xl border border-[#E7EAF0] p-3 text-sm">
                  ...
                  <p className="mt-1"><span className="text-[#667085]">VIN:</span> {selectedVariant.vin || 'Не указан'}</p>
