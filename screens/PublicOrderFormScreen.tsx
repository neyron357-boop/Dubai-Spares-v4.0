...
import { Part, Priority, Source } from '../types';
...
const POPULAR_BRANDS = ['Toyota', 'BMW', 'Mercedes-Benz', 'Ford', 'Lexus', 'Kia', 'Hyundai'];
...
const normalizeDraftRequestedParts = (input: unknown): RequestedPartInput[] => {
  ...
  const normalized = input.map((item): RequestedPartInput | null => {
...
const PublicOrderFormScreen: React.FC = () => {
  ...
  const [contactCountryCode, setContactCountryCode] = useState<string>(PHONE_CODES[0].code);
  ...
  useEffect(() => {
    ...
    try {
      if (window.localStorage.getItem('dubai-spares-run-cloud-diagnostics') !== '1') return;
    } catch {
      return;
    }
...
Best time: ${bestContactTime || '—'}`,
      ...
      try {
        await addOrderItem({
          ...
          parts: partsToInsert,
          markupPercent: 0,
          exchangeRate: Number(settings.defaultExchangeRate || 3.67),
          createdAt: Date.now(),
          isArchived: false,
          isSold: false,
          isLead: true,
  ...
  return (
    <div className="min-h-[100dvh] pb-[calc(104px+env(safe-area-inset-bottom))]" style={{ background: '#0B1220', color: '#F9FAFB', fontFamily: 'Inter, system-ui, sans-serif' }}>
      {import.meta.env.DEV && (() => {
        try {
          return window.localStorage.getItem('dubai-spares-show-public-debug') === '1';
        } catch {
          return false;
        }
      })() && (
